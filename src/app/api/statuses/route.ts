...
export async function GET() {
  ...
  return json(await prisma.status.findMany({ orderBy: [{ rank: "asc" }, { name: "asc" }] }));
}
export async function POST(req: NextRequest) {
  ...
  try {
    return json(await prisma.status.create({ data: { name: p.data.name, rank: p.data.rank, color: p.data.color || null } }), { status: 201 });
