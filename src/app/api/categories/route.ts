...
export async function GET() {
  ...
  return json(await prisma.category.findMany({ orderBy: { name: "asc" } }));
}
export async function POST(req: NextRequest) {
  ...
  try {
    return json(await prisma.category.create({ data: { name: p.data.name, color: p.data.color || null } }), { status: 201 });
